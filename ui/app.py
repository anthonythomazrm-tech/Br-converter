...
from core.output_paths import resolve_output_dir  # NEW
...
VERSION_TEXT = "v1.0 • Developed by Anthony(barriguinha)"
...
class MainApp:
    def __init__(self, window: tb.Window, cfg: ConfigStore):
        ...
        # NEW: salvar na pasta original + subpasta
        self.same_folder_var = tb.BooleanVar(value=bool(cfg.get("last_same_folder", False)))
        self.create_subfolder_var = tb.BooleanVar(value=bool(cfg.get("last_create_subfolder", False)))
        self.subfolder_name_var = tb.StringVar(value=cfg.get("last_subfolder_name", "Convertidos"))
        ...
        # NEW: rows destino (mostrar/ocultar)
        self._dest_row = None
        self._subfolder_row = None
        self._subfolder_name_entry = None

        # NEW: última pasta efetiva
        self.last_effective_out_dir: str = ""
    ...
    def mount(self):
        ...
        tb.Separator(footer).pack(fill=X, pady=(10, 6))

        tb.Label(
            footer,
            text=VERSION_TEXT,  # ou "Developed by Anthony • v1.0"
            font=("Segoe UI", 9),
            bootstyle="secondary"
        ).pack(anchor=W)

        # default de pasta destino (quando NÃO usar mesma pasta)
        ...
        self._refresh_destination_visibility()
    ...
    def _build_docs_tab(self, parent: tb.Frame):
        ...
        # row destino (some quando same_folder=True)
        self._dest_row = tb.Frame(card_cfg)
        self._dest_row.pack(fill=X, pady=(0, 10))
        tb.Label(self._dest_row, text="Salvar no:").pack(side=LEFT)
        tb.Entry(self._dest_row, textvariable=self.output_dir_var).pack(side=LEFT, fill=X, expand=YES, padx=8)
        tb.Button(self._dest_row, text="Procurar", bootstyle=SECONDARY, command=self.pick_dir).pack(side=LEFT)

        # subpasta (só aparece quando same_folder=True)
        self._subfolder_row = tb.Frame(card_cfg)
        self._subfolder_row.pack(fill=X, pady=(0, 6))
        ...
        tb.Label(self._subfolder_row, text="Nome:").pack(side=LEFT, padx=(12, 0))
        self._subfolder_name_entry = tb.Entry(self._subfolder_row, textvariable=self.subfolder_name_var, width=18)
        self._subfolder_name_entry.pack(side=LEFT, padx=8)
    ...
    def open_dest(self):
        d = (self.last_effective_out_dir or self.output_dir_var.get()).strip()
        if d:
            open_folder(d)
    ...
    def _on_same_folder_change(self):
        self.cfg.set("last_same_folder", bool(self.same_folder_var.get()))
        self._refresh_destination_visibility()

    def _on_subfolder_toggle(self):
        self.cfg.set("last_create_subfolder", bool(self.create_subfolder_var.get()))
        self._refresh_destination_visibility()

    def _refresh_destination_visibility(self):
        same = bool(self.same_folder_var.get())

        if self._dest_row:
            if same:
                self._dest_row.pack_forget()
            else:
                self._dest_row.pack(fill=X, pady=(0, 10))

        if self._subfolder_row:
            if same:
                self._subfolder_row.pack(fill=X, pady=(0, 6))
            else:
                self._subfolder_row.pack_forget()

        if self._subfolder_name_entry:
            state = "normal" if (same and bool(self.create_subfolder_var.get())) else "disabled"
            self._subfolder_name_entry.configure(state=state)
    ...
    def run_convert(self):
        ...
        if not self.files:
            Messagebox.show_warning("Selecione arquivos primeiro.", "Aviso")
            return

        chosen_out = self.output_dir_var.get().strip() or None
        same_folder = bool(self.same_folder_var.get())

        out_dir = resolve_output_dir(
            input_path=self.files[0],
            chosen_out_dir=chosen_out,
            same_folder=same_folder,
            create_subfolder=bool(self.create_subfolder_var.get()) if same_folder else False,
            subfolder_name=self.subfolder_name_var.get().strip() or "Convertidos"
        )

        safe_mkdir(out_dir)
        self.last_effective_out_dir = out_dir

        # persistência
        self.cfg.set("last_output_dir", chosen_out or "")
        self.cfg.set("last_target", self.target_var.get())
        self.cfg.set("last_merge", bool(self.merge_var.get()))
        self.cfg.set("last_same_folder", bool(self.same_folder_var.get()))
        self.cfg.set("last_create_subfolder", bool(self.create_subfolder_var.get()))
        self.cfg.set("last_subfolder_name", self.subfolder_name_var.get().strip())
    ...
    def run_image_to_ico(self):
        ...
        out_dir = (self.last_effective_out_dir or self.output_dir_var.get().strip())
        if not out_dir:
